...
public final class LogMath {
    ...
    public static final double LOG_ZERO = -Double.MAX_VALUE;
    public static final double LOG_ONE = 0.d;

    // Singleton instance.
    private static LogMath instance;
    private static double logBase = 1.0001d;
    ...
    private LogMath() {
        ...
        inverseNaturalLogBase = 1.0d / naturalLogBase;
        if (useTable) {
            ...
            entriesInTheAddTable = (int) -Math
                    .rint(linearToLog(logToLinear(0.5d) - 1));
    ...
    private double addTableActualComputation(double index) {
     ...
     */
    private double addTable(double index) throws IllegalArgumentException {
     ...
     *                                  </p>
     */
    public final double subtractAsLinear(double logMinuend, double logSubtrahend)
            throws IllegalArgumentException {
        double logInnerSummation;
    ...
    public final double log10ToLog(double logSource) {
        return logToLog(logSource, 10.0d, logBase);
    }
     ...
     * double. The linear scale numbers can be any double value.
     *
     ...
     * @param out result
     */
    public void logToLinear(double[] vector, double[] out) {
        for (int i = 0; i < vector.length; i++) {
            out[i] = logToLinear(vector[i]);
        }
    }
